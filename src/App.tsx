...
function CarouselItem({ project, index, total, onClick }: any) {
  ...
  return (
    <div 
      ref={ref}
      onClick={onClick}
      className="absolute top-[50vh] -mt-[30vh] cursor-pointer shrink-0 w-[45vw] h-[60vh] flex flex-col justify-center items-center group z-30"
      style={{ pointerEvents: 'auto' }}
    >
      <DOMProxy id={project.id} className="absolute inset-0 w-full h-full" />
      <h2 className="text-white text-4xl font-bold  transition-opacity duration-500 z-10 drop-shadow-lg text-center select-none" style={{ fontFamily: '"Playfair Display", serif' }}>
        {project.title}
      </h2>
    </div>
  );
    ...
    const measure = () => {
      if (ref.current) {
        ...
        if (id === 'project-hero') {
           ...
           absoluteRect.left = rect.left;
           absoluteRect.right = rect.right;
        }
